...
void TreeBillboardsApp::Draw(const GameTimer& gt)
{
	...
	DrawRenderItems(mCommandList.Get(), mRitemLayer[(int)RenderLayer::TreeSprite]);
...
void TreeBillboardsApp::AnimateMaterials(const GameTimer& gt)
{
	...
	water->MatTransform(3, 0) = tu;
	water->MatTransform(3, 1) = tv;
...
void TreeBillboardsApp::LoadTextures()
{
	...
	for (size_t i = 0; i < texNameList.size(); i++)
	{
		...
		ThrowIfFailed(CreateDDSTextureFromFile12(md3dDevice.Get(), mCommandList.Get(),
			texFileNameList[i].c_str(), texMap->Resource, texMap->UploadHeap));
...
void TreeBillboardsApp::BuildRootSignature()
{
	...
	slotRootParameter[3].InitAsDescriptorTable(1, &texTable, D3D12_SHADER_VISIBILITY_PIXEL);		// Texture Map.

	auto staticSampelers = GetStaticSamplers();
	CD3DX12_ROOT_SIGNATURE_DESC rootSigDesc(4, slotRootParameter,
		(UINT)staticSampelers.size(), staticSampelers.data(),
...
void TreeBillboardsApp::BuildDescriptorHeaps()
{
	...
	std::vector<ComPtr<ID3D12Resource>> texList =
	{
		...
		mTextures["treeArrayTex"]->Resource
	};
	...
	for (size_t i = 0; i < texList.size() - 1; i++)
	{
	...
	srvDesc.Format = texList[texList.size() - 1]->GetDesc().Format;
	srvDesc.Texture2DArray.MipLevels = texList[texList.size() - 1]->GetDesc().MipLevels;
	srvDesc.Texture2DArray.MostDetailedMip = 0;
	srvDesc.Texture2DArray.FirstArraySlice = 0;
	srvDesc.Texture2DArray.ArraySize = texList[texList.size() - 1]->GetDesc().DepthOrArraySize;
	md3dDevice->CreateShaderResourceView(texList[texList.size() - 1].Get(), &srvDesc, hDescriptor);
}
...
void TreeBillboardsApp::BuildRenderItems()
{
	...
	treeSpritesRitem->IndexCount = treeSpritesRitem->Geo->DrawArgs["tree"].IndexCount;
	treeSpritesRitem->StartIndexLocation = treeSpritesRitem->Geo->DrawArgs["tree"].StartIndexLocation;
	treeSpritesRitem->BaseVertexLocation = treeSpritesRitem->Geo->DrawArgs["tree"].BaseVertexLocation;
...
void TreeBillboardsApp::BuildPSOs()
{
	...
	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&treeSpritePsoDesc,
		IID_PPV_ARGS(mPSOs["treeSprite"].GetAddressOf())));
}
