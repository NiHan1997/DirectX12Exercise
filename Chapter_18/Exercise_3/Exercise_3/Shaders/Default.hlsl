...
float4 PS (VertexOut pin) : SV_Target
{
	...
	// 添加立方体纹理采样, 这里使用折射效果.
	float3 refractDir = refract(normalize(-toEyeW), pin.NormalW, 1.0/1.5);
	float4 refractColor = gCubeMap.Sample(gsamAnisotropicWrap, refractDir);

	// 计算菲涅尔反射程度.
	float3 fresnelFactor = SchlickFresnel(fresnelR0, pin.NormalW, refractDir);
	finalColor.rgb += gShininess * fresnelFactor * refractColor.rgb;
