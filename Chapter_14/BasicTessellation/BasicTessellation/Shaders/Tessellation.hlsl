...
[maxtessfactor(64.0)]
HollOut HS(InputPatch<VertexOut, 4> p, uint i : SV_OutputControlPointID, uint patchID : SV_PrimitiveID)
{
	HollOut hout;
	hout.PosL = p[i].PosL;
...
[domain("quad")]
DomainOut DS(PatchTess patchTess, float2 uv : SV_DomainLocation, const OutputPatch<HollOut, 4> quad)
{
	DomainOut dout;
