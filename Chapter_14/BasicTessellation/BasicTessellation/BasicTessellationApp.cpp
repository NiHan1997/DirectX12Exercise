...
void BasicTessellationApp::BuildRootSignature()
{
	...
	ComPtr<ID3DBlob> serlizedRootSig = nullptr;
	ComPtr<ID3DBlob> errorBlob = nullptr;
	HRESULT hr = D3D12SerializeRootSignature(&rootSigDesc, D3D_ROOT_SIGNATURE_VERSION_1,
		serlizedRootSig.GetAddressOf(), errorBlob.GetAddressOf());
	if (errorBlob != nullptr) OutputDebugStringA((char*)errorBlob->GetBufferPointer());
	ThrowIfFailed(hr);

	ThrowIfFailed(md3dDevice->CreateRootSignature(0, serlizedRootSig->GetBufferPointer(),
		serlizedRootSig->GetBufferSize(), IID_PPV_ARGS(mRootSignature.GetAddressOf())));
}
...
{
	mShaders["tessVS"] = d3dUtil::CompileShader(L"Shaders/Tessellation.hlsl", nullptr, "VS", "vs_5_1");
	...
	mShaders["tessPS"] = d3dUtil::CompileShader(L"Shaders/Tessellation.hlsl", nullptr, "PS", "ps_5_1");
...
void BasicTessellationApp::DrawRenderItems(ID3D12GraphicsCommandList * cmdList, const std::vector<RenderItem*>& ritems)
{
	...
	for (size_t i = 0; i < ritems.size(); i++)
	{
		...
		CD3DX12_GPU_DESCRIPTOR_HANDLE texHandle(mSrvDescriptorHeap->GetGPUDescriptorHandleForHeapStart(),
			ri->Mat->MatCBIndex, mCbvSrvUavDescriptorSize);
