...
/// 着色器输入纹理资源.
Texture2D gDiffuseMap_1 : register(t0);
Texture2D gDiffuseMap_2 : register(t1);
...
VertexOut VS (VertexIn vin)
{
	...
	vin.TexC = mul(float2(vin.TexC.x, vin.TexC.y), rotation);
	vin.TexC += float2(0.5, 0.5);
...
{
	// 纹理采样, 这里将两张纹理组合在一起.
	float4 diffuseAlbedo = gDiffuseMap_1.Sample(gsamAnisotropicClamp, pin.TexC) *
		gDiffuseMap_2.Sample(gsamAnisotropicClamp, pin.TexC) *
