...
void InstancingAndCullingApp::Draw(const GameTimer & gt)
{
	...
	mCommandList->SetGraphicsRootShaderResourceView(1, mCurrFrameResource->MaterialBuffer->Resource()->GetGPUVirtualAddress());
...
void InstancingAndCullingApp::UpdateInstanceData(const GameTimer & gt)
{
	...
	// 在这里注意和以前的写法不一样, 因为一个渲染项可能有多个实例数据.
	...
	for (auto& e : mAllRitems)
	{
		...
		for (UINT i = 0; i < (UINT)instanceData.size(); ++i)
		{
			...
			if (localSpaceFrustum.Contains(e->Bounds) != DISJOINT)
			{
				...
				// 实例化多少个物体, 就在缓冲区添加多少数据.
				currInstanceBuffer->CopyData(visibleCount++, data);
			}
		}
...
void InstancingAndCullingApp::BuildRootSignature()
{
	...
	diffuseTexTable.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 7, 2, 0);

	CD3DX12_ROOT_PARAMETER slotRootParameter[4];
	slotRootParameter[0].InitAsShaderResourceView(0, 0);		// InstanceData.
	slotRootParameter[1].InitAsShaderResourceView(1, 0);		// MaterialData.
	slotRootParameter[2].InitAsConstantBufferView(0, 0);		// PassCB.
	slotRootParameter[3].InitAsDescriptorTable(1, &diffuseTexTable, D3D12_SHADER_VISIBILITY_PIXEL);
...
void InstancingAndCullingApp::BuildDescriptorHeaps()
{
	...
	const std::array<ComPtr<ID3D12Resource>, 7> tex2DArr =
	{
	...
	for (int i = 0; i < n; ++i)	// y.
	{
		for (int j = 0; j < n; ++j)	// z.
		{
			for (int k = 0; k < n; ++k)	// x.
			{
				...
				XMStoreFloat4x4(&skullRitem->Instances[index].TexTransform,
...
void InstancingAndCullingApp::DrawRenderItems(ID3D12GraphicsCommandList * cmdList, const std::vector<RenderItem*>& ritems)
{
	for (size_t i = 0; i < ritems.size(); i++)
	{
		...
		auto instantceBuffer = mCurrFrameResource->InstanceBuffer->Resource();
		cmdList->SetGraphicsRootShaderResourceView(0, instantceBuffer->GetGPUVirtualAddress());
