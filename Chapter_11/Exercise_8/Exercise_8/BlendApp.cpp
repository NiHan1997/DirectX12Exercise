...
void BlendApp::Draw(const GameTimer& gt)
{
	...
	for (int i = 1; i < 5; ++i)
	{
		...
		mCommandList->SetGraphicsRoot32BitConstant(3, i, 0);
...
void BlendApp::BuildRootSignature()
{
	...
	slotRootParameter[3].InitAsConstants(1, 3, 0, D3D12_SHADER_VISIBILITY_PIXEL);		// Stencil Ref.
	slotRootParameter[4].InitAsDescriptorTable(1, &diffuseTexTable, D3D12_SHADER_VISIBILITY_PIXEL);		// Diffuse Map.
...
void BlendApp::BuildPSOs()
{
	...
	// 绘制通用的模板测试, 将绘制的次数存储在模板值中.
	D3D12_DEPTH_STENCIL_DESC dds;
	dds.DepthEnable = TRUE;
	dds.DepthFunc = D3D12_COMPARISON_FUNC_LESS;
	dds.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ALL;
	dds.StencilEnable = TRUE;
	dds.StencilReadMask = 0xff;
	dds.StencilWriteMask = 0xff;

	dds.FrontFace.StencilDepthFailOp = D3D12_STENCIL_OP_KEEP;
	dds.FrontFace.StencilFailOp = D3D12_STENCIL_OP_KEEP;
	dds.FrontFace.StencilPassOp = D3D12_STENCIL_OP_INCR;
	dds.FrontFace.StencilFunc = D3D12_COMPARISON_FUNC_ALWAYS;

	dds.BackFace.StencilDepthFailOp = D3D12_STENCIL_OP_KEEP;
	dds.BackFace.StencilFailOp = D3D12_STENCIL_OP_KEEP;
	dds.BackFace.StencilPassOp = D3D12_STENCIL_OP_INCR;
	dds.BackFace.StencilFunc = D3D12_COMPARISON_FUNC_EQUAL;
	...
	opaquePsoDesc.DepthStencilState = dds;
	...
	///
	/// 绘制颜色的PSO.
	/// 将模板值与参考值比较, 将绘制不同次数的像素用不同的颜色显示.
	/// 在屏幕后处理效果中, 要禁止深度写入, 防止影响后面物体的渲染, 但是要通过深度测试, 
	/// 让其一直显示在屏幕最前端.
	///
	D3D12_DEPTH_STENCIL_DESC colorDDS;
	colorDDS.DepthEnable = TRUE;
	colorDDS.DepthFunc = D3D12_COMPARISON_FUNC_ALWAYS;
	colorDDS.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ZERO;
	colorDDS.StencilEnable = TRUE;
	colorDDS.StencilReadMask = 0xff;
	colorDDS.StencilWriteMask = 0xff;

	colorDDS.FrontFace.StencilDepthFailOp = D3D12_STENCIL_OP_KEEP;
	colorDDS.FrontFace.StencilFailOp = D3D12_STENCIL_OP_KEEP;
	colorDDS.FrontFace.StencilPassOp = D3D12_STENCIL_OP_KEEP;
	colorDDS.FrontFace.StencilFunc = D3D12_COMPARISON_FUNC_EQUAL;

	colorDDS.BackFace.StencilDepthFailOp = D3D12_STENCIL_OP_KEEP;
	colorDDS.BackFace.StencilFailOp = D3D12_STENCIL_OP_KEEP;
	colorDDS.BackFace.StencilPassOp = D3D12_STENCIL_OP_INCR;
	colorDDS.BackFace.StencilFunc = D3D12_COMPARISON_FUNC_EQUAL;
	...
	colorPsoDesc.DepthStencilState = colorDDS;
	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&colorPsoDesc,
		IID_PPV_ARGS(mPSOs["color"].GetAddressOf())));
}
...
void BlendApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
	...
	for (size_t i = 0; i < ritems.size(); i++)
	{
		...
		cmdList->SetGraphicsRootDescriptorTable(4, texHandle);
...
void BlendApp::DrawFullWindowQuad(ID3D12GraphicsCommandList* cmdList)
{
