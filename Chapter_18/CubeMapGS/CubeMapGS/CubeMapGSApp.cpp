#include "CubeMapGSApp.h"

CubeMapGSApp::CubeMapGSApp(HINSTANCE hInstance) : D3DApp(hInstance)
{
}

CubeMapGSApp::~CubeMapGSApp()
{
}

bool CubeMapGSApp::Initialize()
{
...
void CubeMapGSApp::CreateRtvAndDsvDescriptorHeaps()
{
	// RTV.
...
void CubeMapGSApp::Update(const GameTimer & gt)
{
	OnKeyboardInput(gt);
...
void CubeMapGSApp::Draw(const GameTimer & gt)
{
...
void CubeMapGSApp::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void CubeMapGSApp::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void CubeMapGSApp::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void CubeMapGSApp::OnKeyboardInput(const GameTimer & gt)
{
...
void CubeMapGSApp::AnimateMaterials(const GameTimer & gt)
{
}

void CubeMapGSApp::UpdateObjectCBs(const GameTimer & gt)
{
...
void CubeMapGSApp::UpdateMaterialBuffer(const GameTimer & gt)
{
...
void CubeMapGSApp::UpdateMainPassCB(const GameTimer & gt)
{
	...
	mMainPassCB.EyePosW[0] = { mCamera.GetPosition3f().x, mCamera.GetPosition3f().y, mCamera.GetPosition3f().z, 0.0f };
	mMainPassCB.RenderTargetSize[0] = XMFLOAT4((float)mClientWidth, (float)mClientHeight, 0.0f, 0.0f);
	mMainPassCB.InvRenderTargetSize[0] = XMFLOAT4(1.0f / mClientWidth, 1.0f / mClientHeight, 0.0f, 0.0f);
	mMainPassCB.NearZ[0] = { 0.1f , 0.0f, 0.0f, 0.0f};
	mMainPassCB.FarZ[0] = {1000.0f, 0.0f, 0.0f, 0.0f };
...
void CubeMapGSApp::UpdateCubeMapFacePassCBs()
{
	for (int i = 0; i < 6; ++i)
	{
		...
		XMStoreFloat4x4(&mMainPassCB.View[1 + i], XMMatrixTranspose(view));
		XMStoreFloat4x4(&mMainPassCB.InvView[1 + i], XMMatrixTranspose(invView));
		XMStoreFloat4x4(&mMainPassCB.Proj[1 + i], XMMatrixTranspose(proj));
		XMStoreFloat4x4(&mMainPassCB.InvProj[1 + i], XMMatrixTranspose(invProj));
		XMStoreFloat4x4(&mMainPassCB.ViewProj[1 + i], XMMatrixTranspose(viewProj));
		XMStoreFloat4x4(&mMainPassCB.InvViewProj[1 + i], XMMatrixTranspose(invViewProj));

		mMainPassCB.EyePosW[1 + i] = { mCubeMapCamera[i].GetPosition3f().x, mCubeMapCamera[i].GetPosition3f().y, mCubeMapCamera[i].GetPosition3f().z, 0.0f };;
		mMainPassCB.RenderTargetSize[1 + i] = XMFLOAT4((float)CubeMapSize, (float)CubeMapSize, 0.0f, 0.0f);
		mMainPassCB.InvRenderTargetSize[1 + i] = XMFLOAT4(1.0f / CubeMapSize, 1.0f / CubeMapSize, 0.0f, 0.0f);
	}
}

void CubeMapGSApp::LoadTextures()
{
...
void CubeMapGSApp::BuildRootSignature()
{
	// CubeMap.
...
void CubeMapGSApp::BuildDescriptorHeaps()
{
	///
	/// 创建描述符堆.
	///
	...
	CD3DX12_CPU_DESCRIPTOR_HANDLE cubeRtvHandle(rtvCpuStart, rtvOffset, mRtvDescriptorSize);
...
void CubeMapGSApp::BuildCubeDepthStencil()
{
	...
	md3dDevice->CreateDepthStencilView(mCubeDepthStencilBuffer.Get(), &dsvDesc, mCubeDSV);
}

void CubeMapGSApp::BuildShadersAndInputLayout()
{
	mShaders["standardVS"] = d3dUtil::CompileShader(L"Shaders/Default.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L"Shaders/Default.hlsl", nullptr, "PS", "ps_5_1");

	mShaders["cubeMapVS"] = d3dUtil::CompileShader(L"Shaders/CubeMap.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["cubeMapGS"] = d3dUtil::CompileShader(L"Shaders/CubeMap.hlsl", nullptr, "GS_CubeMap", "gs_5_1");
	mShaders["cubeMapPS"] = d3dUtil::CompileShader(L"Shaders/CubeMap.hlsl", nullptr, "PS", "ps_5_1");
	
	mShaders["cubeMapSkyVS"] = d3dUtil::CompileShader(L"Shaders/CubeMapSky.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["cubeMapSkyGS"] = d3dUtil::CompileShader(L"Shaders/CubeMapSky.hlsl", nullptr, "GS_CubeMap", "gs_5_1");
	mShaders["cubeMapSkyPS"] = d3dUtil::CompileShader(L"Shaders/CubeMapSky.hlsl", nullptr, "PS", "ps_5_1");
...
void CubeMapGSApp::BuildSkullGeometry()
{
...
void CubeMapGSApp::BuildShapeGeometry()
{
...
void CubeMapGSApp::BuildMaterials()
{
...
void CubeMapGSApp::BuildRenderItems()
{
	// 天空盒.
...
void CubeMapGSApp::BuildFrameResources()
{
...
{	
	...
	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&basePsoDesc,
		IID_PPV_ARGS(mPSOs["opaque"].GetAddressOf())));

	///
	/// 几何着色器绘制动态立方体纹理PSO.
	///
	D3D12_GRAPHICS_PIPELINE_STATE_DESC cubeMapPsoDesc = basePsoDesc;
	cubeMapPsoDesc.VS =
	{
		reinterpret_cast<BYTE*>(mShaders["cubeMapVS"]->GetBufferPointer()),
		mShaders["cubeMapVS"]->GetBufferSize()
	};
	cubeMapPsoDesc.GS =
	{
		reinterpret_cast<BYTE*>(mShaders["cubeMapGS"]->GetBufferPointer()),
		mShaders["cubeMapGS"]->GetBufferSize()
	};
	cubeMapPsoDesc.PS =
	{
		reinterpret_cast<BYTE*>(mShaders["cubeMapPS"]->GetBufferPointer()),
		mShaders["cubeMapPS"]->GetBufferSize()
	};
	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&cubeMapPsoDesc,
		IID_PPV_ARGS(mPSOs["cubeMap"].GetAddressOf())));
	...
	/// 绘制反射天空盒的PSO.
	///
	D3D12_GRAPHICS_PIPELINE_STATE_DESC cubeMapSkyPsoDesc = skyPsoDesc;
	cubeMapSkyPsoDesc.VS =
	{
		reinterpret_cast<BYTE*>(mShaders["cubeMapSkyVS"]->GetBufferPointer()),
		mShaders["cubeMapSkyVS"]->GetBufferSize()
	};
	cubeMapSkyPsoDesc.GS =
	{
		reinterpret_cast<BYTE*>(mShaders["cubeMapSkyGS"]->GetBufferPointer()),
		mShaders["cubeMapSkyGS"]->GetBufferSize()
	};
	cubeMapSkyPsoDesc.PS =
	{
		reinterpret_cast<BYTE*>(mShaders["cubeMapSkyPS"]->GetBufferPointer()),
		mShaders["cubeMapSkyPS"]->GetBufferSize()
	};
	cubeMapSkyPsoDesc.RasterizerState.CullMode = D3D12_CULL_MODE_FRONT;
	cubeMapSkyPsoDesc.DepthStencilState.DepthFunc = D3D12_COMPARISON_FUNC_LESS_EQUAL;
	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&cubeMapSkyPsoDesc,
		IID_PPV_ARGS(mPSOs["cubeMapSky"].GetAddressOf())));
}

void CubeMapGSApp::DrawRenderItems(ID3D12GraphicsCommandList * cmdList, const std::vector<RenderItem*>& ritems)
{
...
void CubeMapGSApp::DrawSceneToCubeMap()
{
	...
	/// 绘制几何体和天空盒到动态立方体纹理.
	///
	mCommandList->SetPipelineState(mPSOs["cubeMap"].Get());
	DrawRenderItems(mCommandList.Get(), mRitemLayer[(int)RenderLayer::Opaque]);

	mCommandList->SetPipelineState(mPSOs["cubeMapSky"].Get());
	DrawRenderItems(mCommandList.Get(), mRitemLayer[(int)RenderLayer::Sky]);
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 6> CubeMapGSApp::GetStaticSamplers()
{
...
void CubeMapGSApp::BuildCubeFaceCamera(float x, float y, float z)
{
	...
	for (int i = 0; i < 6; ++i)
	{
		...
		// 这里为了让摄像机能够绘制到外部天空盒, 需要把远平面加大.
		mCubeMapCamera[i].SetLens(0.5f * XM_PI, 1.0f, 0.1f, 5000.0f);
