...
void BlendApp::Draw(const GameTimer& gt)
{
	...
	// 绘制闪电.
	mCommandList->SetPipelineState(mPSOs["lightning"].Get());
	DrawRenderItems(mCommandList.Get(), mRitemLayer[(int)RenderLayer::Lightning]);
...
{
	// 先加载电流的纹理.
	std::vector<std::string> lightningTexNameList(60);
	std::vector<std::wstring> lightningTexFileNameList(60);
	for (size_t i = 0; i < 60; i++)
	{
		if (i < 9)
		{
			lightningTexNameList[i] = "lightningTex_0" + std::to_string(i + 1);
			lightningTexFileNameList[i] = L"Textures/Bolt00" + std::to_wstring(i + 1) + L".DDS";
		}
		else
		{
			lightningTexNameList[i] = "lightningTex_" + std::to_string(i + 1);
			lightningTexFileNameList[i] = L"Textures/Bolt0" + std::to_wstring(i + 1) + L".DDS";
		}
	}
	...
	for (int i = 0; i < 60; ++i)
	{
		...
		texMap->Name = lightningTexNameList[i];
		texMap->Filename = lightningTexFileNameList[i];
...
void BlendApp::BuildSrvHeaps()
{
	...
	std::vector<ComPtr<ID3D12Resource>> tex2DList(62);
	for (int i = 0; i < 60; ++i)
	{
		if (i < 9)
			tex2DList[i] = mTextures["lightningTex_0" + std::to_string(i + 1)]->Resource;
		else
			tex2DList[i] = mTextures["lightningTex_" + std::to_string(i + 1)]->Resource;
	};
	tex2DList[60] = mTextures["grassTex"]->Resource;
	tex2DList[61] = mTextures["waterTex"]->Resource;
...
void BlendApp::BuildLightningGeo()
{
	GeometryGenerator::MeshData lightning = CreateCylinderWidthoutTopAndBottom(5.0f, 5.0f, 15.0f, 20, 20);

	std::vector<Vertex> vertices(lightning.Vertices.size());
	for (size_t i = 0; i < lightning.Vertices.size(); i++)
	{
		vertices[i].Position = lightning.Vertices[i].Position;
		vertices[i].Normal = lightning.Vertices[i].Normal;
		vertices[i].TexC = lightning.Vertices[i].TexC;
	}

	std::vector<std::uint16_t> indices = lightning.GetIndices16();

	const UINT vbByteSize = (UINT)vertices.size() * sizeof(Vertex);
	const UINT ibByteSize = (UINT)indices.size() * sizeof(std::uint16_t);

	auto geo = std::make_unique<MeshGeometry>();
	geo->Name = "lightningGeo";
	...
	geo->DrawArgs["lightning"] = submesh;
	mGeometries[geo->Name] = std::move(geo);
}
...
void BlendApp::BuildMaterials()
{
	...
	lightning->MatCBIndex = 2;
	lightning->DiffuseSrvHeapIndex = 0;
	lightning->DiffuseAlbedo = { 1.0f, 1.0f, 1.0f, 0.5f };
	lightning->FresnelR0 = { 0.1f, 0.1f, 0.1f };
	lightning->Roughness = 0.125f;

	mMaterials[grass->Name] = std::move(grass);
	mMaterials[water->Name] = std::move(water);
	mMaterials[lightning->Name] = std::move(lightning);
}
...
void BlendApp::BuildRenderItems()
{
	...
	auto lightningRitem = std::make_unique<RenderItem>();
	XMStoreFloat4x4(&lightningRitem->World, XMMatrixTranslation(3.0f, 10.0f, -9.0f));
	XMStoreFloat4x4(&lightningRitem->TexTransform, XMMatrixScaling(1.0f, 2.0f, 1.0f));
	lightningRitem->ObjectCBIndex = 2;
	lightningRitem->Mat = mMaterials["lightning"].get();
	lightningRitem->Geo = mGeometries["lightningGeo"].get();
	lightningRitem->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	lightningRitem->IndexCount = lightningRitem->Geo->DrawArgs["lightning"].IndexCount;
	lightningRitem->StartIndexLocation = lightningRitem->Geo->DrawArgs["lightning"].StartIndexLocation;
	lightningRitem->BaseVertexLocation = lightningRitem->Geo->DrawArgs["lightning"].BaseVertexLocation;
	mRitemLayer[(int)RenderLayer::Lightning].push_back(lightningRitem.get());

	mAllRitems.push_back(std::move(wavesRitem));
	mAllRitems.push_back(std::move(landRitem));
	mAllRitems.push_back(std::move(lightningRitem));
}
...
void BlendApp::BuildPSOs()
{
	...
	// 闪电的PSO.
	D3D12_GRAPHICS_PIPELINE_STATE_DESC lightningPsoDesc = transparentPsoDesc;
	lightningPsoDesc.RasterizerState.CullMode = D3D12_CULL_MODE_NONE;
	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&lightningPsoDesc,
		IID_PPV_ARGS(mPSOs["lightning"].GetAddressOf())));
}

GeometryGenerator::MeshData BlendApp::CreateCylinderWidthoutTopAndBottom(float bottomRadius, float topRadius, float height, std::uint32_t sliceCount, std::uint32_t stackCount)
{
...
void BlendApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
	...
	for (size_t i = 0; i < ritems.size(); i++)
	{
		...
		if (ri->Mat->DiffuseSrvHeapIndex == 0)		
			texHandle.Offset((lightningMapIndex++) % 60, mCbvSrvUavDescriptorSize);
		else 
