...
void SSAOApp::UpdateSkullRitem(const GameTimer& gt)
{
	mAnimationTimePos += gt.DeltaTime();
	if (mAnimationTimePos > mSkullAnimation.GetEndTime()) mAnimationTimePos = 0.0f;

	mSkullAnimation.Interpolate(mAnimationTimePos, mSkullWorld);
	mSkullRitem->World = mSkullWorld;
	mSkullRitem->NumFramesDirty = gNumFrameResources;
}

void SSAOApp::UpdateLightDir(const GameTimer& gt)
{	
	///
	/// 旋转光源.
	///
