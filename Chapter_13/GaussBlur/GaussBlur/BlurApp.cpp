...
void BlurApp::BuildPostProcessRootSignature()
{
	...
	slotRootParameter[0].InitAsConstants(12, 0, 0);				// 常量数据.
...
void BlurApp::BuildDescriptorHeaps()
{
	...
	descHeapDesc.NumDescriptors = textureCount + blurDescCount;
	...
	mBlurFilter->BuildDescriptors(
		CD3DX12_CPU_DESCRIPTOR_HANDLE(mCbvSrvUavDescriptorHeap->GetCPUDescriptorHandleForHeapStart(),
			textureCount, mCbvSrvUavDescriptorSize),
...
void BlurApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
	...
	for (size_t i = 0; i < ritems.size(); i++)
	{
		...
		auto matyCBAddr = mCurrFrameResource->MaterialCB->Resource()->GetGPUVirtualAddress() +
			ri->Mat->MatCBIndex * matCBByteSize;

		cmdList->SetGraphicsRootConstantBufferView(0, objCBAddr);
		cmdList->SetGraphicsRootConstantBufferView(1, matyCBAddr);

		CD3DX12_GPU_DESCRIPTOR_HANDLE texHandle(mCbvSrvUavDescriptorHeap->GetGPUDescriptorHandleForHeapStart());
