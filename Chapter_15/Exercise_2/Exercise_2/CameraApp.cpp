...
CameraApp::CameraApp(HINSTANCE hInstance) : D3DApp(hInstance)
{
}
...
void CameraApp::Draw(const GameTimer & gt)
{
	...
	DrawRenderItems(mCommandList.Get(), mRitemLayer[(int)RenderLayer::Opauqe]);
...
{
	const std::array<std::string, 4> texNameArr = 
	{
		"bricksTex",
		"stoneTex",
		"tileTex",
	...
	const std::array<std::wstring, 4> texFileNameArr =
	{
...
{
	// 将纹理存储在数组中(不是纹理数组), 使用动态索引访问.
	CD3DX12_DESCRIPTOR_RANGE diffuseTexTable;
	diffuseTexTable.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 4, 0, 1);
...
void CameraApp::BuildDescriptorHeaps()
{
	...
	std::array<ComPtr<ID3D12Resource>, 4> tex2DArr =
	{
...
void CameraApp::BuildRenderItems()
{
	...
	for (int i = 0; i < 5; ++i)
	{
		...
		XMStoreFloat4x4(&leftCylRitem->World, leftCylinderWorld);
		...
		XMStoreFloat4x4(&rightCylRitem->World, rightCylinderWorld);
	...
	for (auto& e : mAllRitems)
		mRitemLayer[(int)RenderLayer::Opauqe].push_back(e.get());
}
