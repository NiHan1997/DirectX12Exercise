...
void ShapesApp::Update(const GameTimer& gt)
{
	...
	mCurrFrameResouceIndex = (mCurrFrameResouceIndex + 1) % gNumFrameResources;
	mCurrFrameResource = mFrameResouces[mCurrFrameResouceIndex].get();
...
void ShapesApp::Draw(const GameTimer& gt)
{
	...
	auto passCBHandle = CD3DX12_GPU_DESCRIPTOR_HANDLE(mCbvHeap->GetGPUDescriptorHandleForHeapStart());
	passCBHandle.Offset(mCurrFrameResouceIndex, mCbvSrvUavDescriptorSize);
	mCommandList->SetGraphicsRootDescriptorTable(1, passCBHandle);
...
void ShapesApp::BuildRootSignature()
{
	...
	passCBTable.Init(D3D12_DESCRIPTOR_RANGE_TYPE_CBV, 1, 1, 0);

	CD3DX12_ROOT_PARAMETER slotRootParamter[2];
	slotRootParamter[0].InitAsConstants(16, 0, 0);
	slotRootParamter[1].InitAsDescriptorTable(1, &passCBTable);

	CD3DX12_ROOT_SIGNATURE_DESC rootSigDesc(2, slotRootParamter, 0, nullptr,
...
void ShapesApp::BuildConstantsBuffers()
{
	...
	{
		auto passCBAddr = mFrameResouces[frameIndex]->PassCB->Resource()->GetGPUVirtualAddress();
...
void ShapesApp::BuildShapesGeo()
{
	...
	UINT gridIndexOffset = (UINT)box.GetIndices16().size();
	UINT sphereIndexOffset = (UINT)grid.GetIndices16().size() + gridIndexOffset;
	UINT cylinderIndexOffset = (UINT)sphere.GetIndices16().size() + sphereIndexOffset;
	...
	gridSubmesh.IndexCount = (UINT)grid.GetIndices16().size();
	gridSubmesh.StartIndexLocation = gridIndexOffset;
	gridSubmesh.BaseVertexLocation = gridVertexOffset;

	SubmeshGeometry sphereSubmesh;
	sphereSubmesh.IndexCount = (UINT)sphere.GetIndices16().size();
	sphereSubmesh.StartIndexLocation = sphereIndexOffset;
	sphereSubmesh.BaseVertexLocation = sphereVertexOffset;

	SubmeshGeometry cylinderSubmesh;
	cylinderSubmesh.IndexCount = (UINT)cylinder.GetIndices16().size();
...
void ShapesApp::BuildSkullGeo()
{
	...
	for (UINT i = 0; i < vcount; ++i)
	{
		...
		vertices[i].Color = XMFLOAT4(Colors::Brown);
	}
	fin >> ignore >> ignore >> ignore;
...
void ShapesApp::BuildRenderItems()
{
	...
	for (int i = 0; i < 5; ++i)
	{
		...
		leftCylRitem->Geo = mGeometries["shapesGeo"].get();
		...
		rightCylRitem->Geo = mGeometries["shapesGeo"].get();
...
void ShapesApp::BuildFrameResources()
{
	...
	{
		mFrameResouces.push_back(std::make_unique<FrameResource>(md3dDevice.Get(), 1,
...
void ShapesApp::BuildPSOs()
{
	...
	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&opaqueWireframePsoDesc,
		IID_PPV_ARGS(mPSOs["opaque_wireframe"].GetAddressOf())));
}
...
void ShapesApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
	for (size_t i = 0; i < ritems.size(); i++)
	{
		...
		// 将世界矩阵改为根常量.
		std::vector<float> world(16);
		for (int i = 0; i < 4; ++i)
		{
			for (int j = 0; j < 4; ++j)
			{
				world[4 * i + j] = ri->World(i, j);
			}
		}
		mCommandList->SetGraphicsRoot32BitConstants(0, 16, world.data(), 0);
