...
VertexOut VS (VertexIn vin)
{
	...
	float3 binormal = cross(normalW, tangentW);
...
float4 PS (VertexOut pin) : SV_Target
{
	...
	// 两个方向运动的法线, 模拟波浪翻滚的效果.
	float4 normalSample1 = gTexMap[normalMapIndex].Sample(gsamAnisotropicWrap,
		pin.TexC + gPassConstants.gTotalTime * 0.1);
	float4 normalSample2 = gTexMap[normalMapIndex + 1].Sample(gsamAnisotropicWrap, 
		pin.TexC - gPassConstants.gTotalTime * 0.1);
	
	// 切线空间-->世界空间.
	float3 normalW1 = 2.0 * normalSample1.rgb - 1.0;
	float3 normalW2 = 2.0 * normalSample2.rgb - 1.0;

	// 大波浪法线设定高一点.
	float3x3 tangentToWorld = float3x3(pin.TtoWorld0.xyz, pin.TtoWorld1.xyz, pin.TtoWorld2.xyz);
	float3 normalW = normalize(mul(normalW1 + normalW2, tangentToWorld));
	...
	float4 dirLight = ComputeLighting(gPassConstants.gLights, mat, posW.xyz, normalW, toEyeW, 1.0);
